...
updates_dir=/data/orion_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have orion-VERSION-DATE-TYPE-* as format"
...
# Assume orion-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.orion.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.orion.updater/databases/updates.db" \
